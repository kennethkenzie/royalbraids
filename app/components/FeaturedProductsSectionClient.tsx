...
function ProductCard({ product }: { product: Product }) {
  return (
    ...
    >
      <Link href={`/products/${product.slug}`} className="block relative aspect-[4/5] overflow-hidden bg-zinc-100 rounded-none">
        <div className="absolute left-4 top-4 z-10">
          <span className="bg-black text-white px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest shadow-lg">
            Featured
          </span>
        </div>
      ...
      <div className="mt-6 flex flex-col items-center text-center">
        {product.category?.name && (
          <p className="text-[12px] font-medium tracking-[0.2em] text-zinc-500 uppercase mb-2 italic">
            {product.category.name}
          </p>
        )}
        <h3 className="text-[18px] font-bold tracking-tight text-black flex-1 line-clamp-2 uppercase">
        ...
        <p className="mt-4 text-[20px] font-black tracking-tighter text-black">
          UGX {(product.priceInCents / 1).toLocaleString()}
        </p>
...
export default function FeaturedProductsSectionClient({ products }: { products: Product[] }) {
  return (
    <section className="w-full bg-white px-5 py-24 md:px-7 md:py-32 xl:px-8">
      <div className="mx-auto max-w-full">
        <div className="mb-16 flex flex-col items-start text-left">
          ...
          >
             <span className="text-[14px] font-bold tracking-[0.4em] text-zinc-400 uppercase">Premium Selection</span>
          </motion.div>
          ...
          >
            Handpicked bestsellers and signature styles chosen by our experts to elevate your lifestyle.
          </motion.p>
        </div>
        ...
        <div className="mt-20 flex justify-center">
          ...
          >
            <span className="text-[14px] font-bold uppercase tracking-[0.3em] text-black">View All Products</span>
